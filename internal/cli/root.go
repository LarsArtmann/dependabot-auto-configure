...
func newRootCmd() (*cobra.Command, *int) {
	...
	rootCmd.Flags().BoolVar(&check, "check", false,
		"report pending changes without writing; exit 1 when the --fail-on policy is exceeded")
	...
	rootCmd.Flags().BoolVar(&secFixes, "enable-security-fixes", false,
		"also enable Dependabot security updates via the GitHub API (needs GITHUB_TOKEN/GH_TOKEN)")
	rootCmd.Flags().StringVar(&failOn, "fail-on", "any",
		"under --check, the minimum finding severity that fails the run: "+
			"any (default), none, or a severity (error, warning, info, critical)")
